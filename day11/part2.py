...
class Monkey:
    ...
    def run(self, monkeys: dict, lcm: int):
        for old in self.items:
            ...
            # ? this might be the slow operation
            # * fast check with pascal's divisibility rule
            if new % self.test == 0:
...
def compute(s: str) -> int:
    ...
    for i in range(10_000):
        for _, monkey in monkeys.items():
            monkey.run(monkeys, math.lcm(*tests))
    
    monkey_numbers = [monkey.items_inspected for _, monkey in monkeys.items()]
    monkey_numbers = sorted(monkey_numbers)
    print(monkey_numbers)
    return monkey_numbers[-1] * monkey_numbers[-2]
